...
import { ChatCITLogo, SpinningGear } from "./ui/helpers";
import { User, Chat } from "../types";
import { API_URL } from "../config";

export function AuthScreen({ dark, onSuccess, onGuest }: { dark: boolean, onSuccess: (u: User, c: Chat[]) => void, onGuest: () => void }) {
  ...
  // Focus states to trigger the security popup
  const [pwdFocused, setPwdFocused] = useState(false);
  const [confirmFocused, setConfirmFocused] = useState(false);

  const bg = dark ? "#1c1b22" : "#f8f9fa";
  const cardBg = dark ? "#25242c" : "#ffffff";
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', width: '100%', background: bg, padding: 24, overflow: 'hidden' }}>
      
      <div style={{ marginBottom: 32 }}>
        <ChatCITLogo dark={dark} size={36} />
      </div>

      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', maxWidth: 400 }}>
        
        {/* --- FLOATING SECURITY REQUIREMENTS (Only shows when focused) --- */}
        {showSecurityPopup && (
          <div style={{
            position: 'absolute',
            right: 'calc(100% + 24px)', 
        ...
        {/* --- MAIN AUTH CARD --- */}
        <div style={{ width: '100%', background: cardBg, border: `1px solid ${border}`, borderRadius: 16, padding: 32, boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '24px 0' }}>
            ...
            <span style={{ fontSize: 11, color: textMuted, fontWeight: 600, textTransform: 'uppercase' }}>OR</span>
            <div style={{ flex: 1, height: 1, background: border }}></div>
          </div>

          <button onClick={onGuest} style={{ width: '100%', padding: '12px', borderRadius: 8, border: `1px solid ${border}`, background: 'transparent', color: textPrimary, fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = dark ? 'rgba(255,255,255,0.05)' : '#f3f4f6'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            Continue as Guest
          </button>
