    ...
    <div style={{ maxWidth: 1400, width: "100%", margin: "0 auto", padding: "32px 16px" }}>
      ...
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: bg, border: `1px solid ${border}`, padding: "8px 14px", borderRadius: 8, flex: 1, minWidth: 250 }}>
          ...
          <input type="text" placeholder={`Search ${activeTab}...`} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ border: "none", background: "transparent", outline: "none", width: "100%", color: "inherit", fontSize: 14 }} />
        </div>
        ...
        {activeTab === 'knowledge' && editingId !== 0 && (
          <button onClick={() => { setEditingId(0); setForm({ keyword: "", response: "", picture_url: "", category: "Handbook", subcategory: "All", display_name: "" }); setKeywordInput(""); }} style={{ display: "flex", alignItems: "center", gap: 6, background: "#4285f4", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontWeight: 500, whiteSpace: "nowrap" }}>
          ...
          {editingId !== null && (
            <div style={{ background: bg, padding: 20, borderRadius: 12, border: `1px solid ${border}`, marginBottom: 24, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value, subcategory: "All" })} style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: `1px solid ${border}`, background: dark ? "rgba(255,255,255,0.05)" : "#f3f4f6", color: "inherit", outline: "none" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 8 }}>
                ...
                <select value={form.subcategory || "All"} onChange={e => setForm({ ...form, subcategory: e.target.value })} style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: `1px solid ${border}`, background: dark ? "rgba(255,255,255,0.05)" : "#f3f4f6", color: "inherit", outline: "none" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <div style={{ padding: "8px 12px", borderRadius: 8, border: `1px solid ${border}`, background: "transparent", display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
                  {keywordsList.map(kw => (<span key={kw} style={{ background: "#4285f4", color: "#fff", padding: "4px 10px", borderRadius: 16, fontSize: 12, fontWeight: 500, display: "flex", alignItems: "center", gap: 6 }}>{kw} <X size={12} cursor="pointer" onClick={() => removeKeyword(kw)} style={{ opacity: 0.8 }} /></span>))}
                  <input value={keywordInput} onChange={e => setKeywordInput(e.target.value)} onKeyDown={handleAddKeyword} placeholder={keywordsList.length === 0 ? "e.g. grading system, passing score" : "Add another..."} style={{ flex: 1, minWidth: 150, border: "none", background: "transparent", color: "inherit", outline: "none", fontSize: 14 }} />
                </div>
              </div>
                  ...
                  {filteredData.map((row: any) => (
                    <tr key={row.id} style={{ borderBottom: `1px solid ${border}` }}>
                      ...
                      <td style={{ padding: "14px 16px", verticalAlign: "top", opacity: 0.9, lineHeight: 1.5 }}>
                        ...
                        {row.picture_url && (row.picture_url.toLowerCase().includes('.pdf') ? (<a href={row.picture_url} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#4285f4", textDecoration: "none", display: "inline-flex", alignItems: "center", padding: "4px 8px", background: dark ? "rgba(255,255,255,0.08)" : "#f3f4f6", borderRadius: 6, fontWeight: 500 }}>📄 Attached PDF</a>) : (<button onClick={() => setFullScreenMedia(row.picture_url!)} style={{ display: "inline-block", background: "none", border: "none", padding: 0, cursor: "zoom-in", marginTop: 4 }} title="Click to enlarge"><img src={row.picture_url} alt="Attached" style={{ height: 48, borderRadius: 6, border: `1px solid ${border}`, objectFit: 'cover' }} /></button>))}
                      </td>
                      <td style={{ padding: "14px 16px", verticalAlign: "top", textAlign: "right", whiteSpace: "nowrap" }}>
                        <button onClick={() => { setForm({ keyword: row.keyword, response: row.response, picture_url: row.picture_url || "", category: row.category || "Handbook", subcategory: row.subcategory || "All", display_name: row.display_name || "" }); setKeywordInput(""); setEditingId(row.id); }} style={{ background: "none", border: "none", color: "#4285f4", cursor: "pointer", padding: 6 }} title="Edit"><Edit2 size={16} /></button>
