...
const GlobalKioskStyles = ({ dark, theme }: { dark: boolean, theme: any }) => (
  <style>{`
    ...
    .bg-blob-2 {
      position: absolute; bottom: 10%; right: -20%; width: 70vw; height: 70vw;
...
const getIconForCategory = (cat: string, size = 20) => {
  ...
  if (lower.includes("org") || lower.includes("affair")) return <Users size={size} />;
...
export const KioskScreen = ({ dark, screenState, setScreenState, kioskCategory, setKioskCategory, kioskResult, setKioskResult, handleKioskSelection, topRightButtons, setFullScreenMedia, setShowCalendar, kioskMapping, setKioskMapping, allSidebarCategories, isAdmin }: any) => {
  ...
  const clusterItems = [
    ...
    { label: "Curriculum and Extensions", alias: "Majors", icon: <FileText size={36} /> }
  ];
      ...
      <div className="screensaver-fullscreen">
        ...
        <div style={{ position: 'absolute', top: 24, left: 32, zIndex: 100, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={goHome}>
            ...
            <span style={{ fontSize: 28, fontWeight: 800, letterSpacing: '-1px' }}>
                <span style={{ color: dark ? '#fff' : '#0f172a' }}>in</span>
                <span style={{ color: '#1257AC' }}>CIT</span>
                <span style={{ color: dark ? '#fff' : '#0f172a' }}>e</span>
            </span>
        </div>
          ...
          <div className="kiosk-main-scroll no-scrollbar">
            ...
            <div className="omantel-grid">
               {clusterItems.map((item, idx) => (
                  <div key={idx} onClick={() => handleKioskSelection("Directory", item.alias)} className="omantel-card" style={{ background: theme.card, borderColor: dark ? theme.border : theme.cardBorder, borderWidth: dark ? 1 : 2, borderStyle: 'solid' }}>
            ...
            {/* MARQUEE & CENTER LOGO */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '48px 0', width: '100%', maxWidth: 640, position: 'relative' }}>
            ...
            <div style={{ width: '100%', padding: '0 24px', position: 'absolute', bottom: 32, maxWidth: 640 }}>
               <div className="omantel-card wide kiosk-pulse-btn" onClick={() => { setScreenState("chat"); setKioskCategory(null); setKioskResult(null); }} style={{ background: dark ? theme.card : theme.cardBorder, color: '#fff', margin: 0, padding: 24, minHeight: 160, display: 'flex', justifyContent: 'center' }}>
                  ...
                  <Bot size={160} style={{ position: 'absolute', right: -20, bottom: -20, opacity: 0.15 }} />
               </div>
            </div>
          </div>
        )}
                ...
                <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '40px', display: 'flex', flexDirection: 'column' }}>
                  ...
                  {kioskResult?.loading ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 16, marginTop: 60 }}>
                      ...
                      <span style={{ fontSize: 24, color: theme.textMuted, fontWeight: 600 }}>inCITe is fetching details...</span>
                    </div>
                  ) : (
