...
const GlobalKioskStyles = ({ dark, theme }: { dark: boolean, theme: any }) => (
  <style>{`
    ...
    .screensaver-fullscreen { position: absolute; top: 0; left: 0; right: 0; bottom: 0; z-index: 999995; background: ${theme.bg}; display: flex; flex-direction: column; align-items: center; justify-content: center; animation: fadeIn 0.4s ease; border-radius: inherit; overflow: hidden; }
    .bg-blob-1 { position: absolute; top: 0%; left: -20%; width: 70vw; height: 70vw; background: ${dark ? 'radial-gradient(circle, rgba(18, 87, 172, 0.25) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(166, 1, 18, 0.12) 0%, transparent 70%)'}; border-radius: 50%; z-index: 0; filter: blur(80px); pointer-events: none; animation: floatBg 20s ease-in-out infinite; }
    .bg-blob-2 { position: absolute; bottom: 0%; right: -20%; width: 80vw; height: 80vw; background: ${dark ? 'radial-gradient(circle, rgba(253, 181, 28, 0.15) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(245, 170, 42, 0.15) 0%, transparent 70%)'}; border-radius: 50%; z-index: 0; filter: blur(100px); pointer-events: none; animation: floatBg 25s ease-in-out infinite reverse; }
    .kiosk-main-scroll { position: relative; z-index: 10; width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding-top: 100px; overflow-y: auto; overflow-x: hidden; padding-bottom: 220px; }
    .greeting-box { display: flex; align-items: center; justify-content: space-between; padding: 24px 32px; border-radius: 32px; background: ${dark ? 'rgba(18, 87, 172, 0.15)' : 'rgba(255, 255, 255, 0.6)'}; border: 1px solid ${dark ? 'rgba(255, 255, 255, 0.1)' : 'rgba(166, 1, 18, 0.2)'}; box-shadow: 0 20px 40px rgba(0,0,0,0.1); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); width: 90%; max-width: 680px; flex-shrink: 0; margin-bottom: 32px; animation: slideDown 0.6s cubic-bezier(0.2, 0.8, 0.2, 1); }
    .omantel-grid { display: grid; grid-template-columns: repeat(6, 1fr); gap: 16px; width: 100%; max-width: 680px; padding: 0 16px; flex-shrink: 0; animation: slideUp 0.8s cubic-bezier(0.2, 0.8, 0.2, 1) both; }
    .glassy-cluster-card { border-radius: 28px; padding: 20px; display: flex; flex-direction: column; background: ${dark ? 'rgba(255, 255, 255, 0.03)' : 'rgba(255, 255, 255, 0.7)'}; border: 1px solid ${dark ? 'rgba(255, 255, 255, 0.1)' : 'rgba(166, 1, 18, 0.15)'}; box-shadow: 0 16px 40px rgba(0,0,0,0.08); backdrop-filter: blur(32px); -webkit-backdrop-filter: blur(32px); cursor: pointer; transition: all 0.3s cubic-bezier(0.2, 0.8, 0.2, 1); position: relative; overflow: hidden; min-height: 160px; }
    .glassy-cluster-card:active { transform: scale(0.95); background: ${dark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.9)'}; }
    .kiosk-pulse-btn { animation: pulse-attention 2s infinite cubic-bezier(0.4, 0, 0.2, 1); background: ${dark ? 'rgba(18, 87, 172, 0.9)' : theme.cardBorder} !important; color: #fff !important; border: none !important; }
    @keyframes pulse-attention { 0% { transform: scale(1); box-shadow: 0 0 0 0 ${dark ? 'rgba(18, 87, 172, 0.8)' : 'rgba(166, 1, 18, 0.6)'}; } 50% { transform: scale(1.02); box-shadow: 0 0 0 25px rgba(253, 181, 28, 0); } 100% { transform: scale(1); box-shadow: 0 0 0 0 rgba(253, 181, 28, 0); } }
    @keyframes floatBg { 0%, 100% { transform: translateY(0) scale(1); } 50% { transform: translateY(-40px) scale(1.05); } }
    @keyframes slideDown { from { opacity: 0; transform: translateY(-30px); } to { opacity: 1; transform: translateY(0); } }
    @keyframes slideUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
    .marquee-container { width: 100%; overflow: hidden; white-space: nowrap; position: relative; padding: 12px; border-radius: 12px; border: 1px solid; }
    .marquee-text { display: inline-block; animation: marquee 35s linear infinite; font-weight: 800; font-size: 16px; letter-spacing: 2px; text-transform: uppercase; }
    @keyframes marquee { 0% { transform: translateX(50%); } 100% { transform: translateX(-100%); } }
    .carousel-container { width: 100%; overflow: hidden; position: relative; -webkit-mask-image: linear-gradient(to right, transparent, black 5%, black 95%, transparent); mask-image: linear-gradient(to right, transparent, black 5%, black 95%, transparent); }
    ...
    .glassy-dir-card { background: ${dark ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.7)'}; border: 1px solid ${dark ? 'rgba(255,255,255,0.1)' : 'rgba(166, 1, 18, 0.15)'}; border-radius: 20px; padding: 24px; display: flex; align-items: center; gap: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.05); cursor: pointer; transition: all 0.2s cubic-bezier(0.2, 0.8, 0.2, 1); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); }
    ...
    .back-btn-modern { display: flex; align-items: center; gap: 8px; background: ${dark ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.8)'}; border: 1px solid ${dark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}; color: ${theme.text}; padding: 12px 20px; border-radius: 24px; font-size: 16px; font-weight: 700; cursor: pointer; transition: transform 0.1s; box-shadow: 0 8px 24px rgba(0,0,0,0.1); backdrop-filter: blur(10px); }
    .back-btn-modern:active { transform: scale(0.92); }
    .kiosk-detail-card { width: 90%; max-width: 860px; flex: 1 1 auto; min-height: 800px; max-height: 85vh; background: ${dark ? 'rgba(18, 87, 172, 0.15)' : 'rgba(255,255,255,0.85)'}; border-radius: 32px; border: 1px solid ${dark ? 'rgba(255,255,255,0.1)' : 'rgba(166, 1, 18, 0.2)'}; display: flex; flex-direction: column; box-shadow: 0 30px 60px rgba(0,0,0,0.3); overflow: hidden; margin-bottom: 24px; z-index: 10; backdrop-filter: blur(32px); -webkit-backdrop-filter: blur(32px); }
...
export const KioskScreen = ({ dark, screenState, setScreenState, kioskCategory, setKioskCategory, kioskResult, setKioskResult, handleKioskSelection, topRightButtons, setFullScreenMedia, setShowCalendar, kioskMapping, screensaverSlides }: any) => {
  ...
  const theme = dark ? { bg: '#1C1D55', card: '#1257AC', accent: '#FDB51C', text: '#ffffff', textMuted: 'rgba(255,255,255,0.7)', border: 'rgba(255,255,255,0.15)' } 
  ...
  const defaultPresentationSlides = [
  ...
  const activeSlides = screensaverSlides && screensaverSlides.length > 0 ? screensaverSlides : defaultPresentationSlides;
  ...
  useEffect(() => {
    if (screenState === 'presentation') { const timer = setInterval(() => setCurrentSlide(s => (s + 1) % activeSlides.length), 6000); return () => clearInterval(timer); }
  }, [screenState, activeSlides.length]);

  useEffect(() => {
     if (kioskResult?.isPdf) setPdfPage(1);
     if (kioskResult?.isDirectory) {
         setDirMajor(kioskResult.subcategory && kioskResult.subcategory !== 'All' ? kioskResult.subcategory : null); setDirPage(1); setLoadingDir(true);
         fetch(`${API_URL}/knowledge`).then(res => res.json()).then(data => {
             ...
             const directoryItems = rawData.filter((item: any) => { const itemCat = (item.category || "").toLowerCase(); const itemSub = (item.subcategory || "").toLowerCase(); return itemCat === targetName || itemSub === targetName; });
             setDbDirectoryData(directoryItems);
           }).catch(err => console.error(err)).finally(() => setLoadingDir(false));
     }
  }, [kioskResult?.title, kioskResult?.isDirectory, API_URL]);

  useEffect(() => {
    if (!kioskResult?.isPdf || !kioskResult?.pdfUrl) return; let isMounted = true;
    const loadPDF = async () => {
      setPdfLoading(true);
      ...
      try { const cleanUrl = kioskResult.pdfUrl.split('#')[0]; const pdf = await (window as any).pdfjsLib.getDocument(cleanUrl).promise; if(isMounted) { setPdfRef(pdf); setTotalPages(pdf.numPages); setPdfPage(1); } } catch(e) { console.error("Failed to load Kiosk PDF", e); } finally { if(isMounted) setPdfLoading(false); }
    };
  ...
  const formatText = (text: string) => {
    ...
    return parts.map((part, i) => { if (part.startsWith('**') && part.endsWith('**')) { return <strong key={i} style={{ color: dark ? '#fff' : '#000', fontWeight: 800 }}>{part.slice(2, -2)}</strong>; } return <span key={i}>{part}</span>; });
  };

  const filteredDirectory = React.useMemo(() => {
      return dbDirectoryData.filter(item => { const major = item.subcategory || "All"; return dirMajor ? major.toLowerCase() === dirMajor.toLowerCase() : true; }).sort((a, b) => { const nameA = a.display_name || (a.keyword ? a.keyword.split(',')[0] : "") || ""; const nameB = b.display_name || (b.keyword ? b.keyword.split(',')[0] : "") || ""; return nameA.localeCompare(nameB); });
  ...
  if (clusterItems.some(c => c.label === kioskCategory)) itemsToRender = kioskMapping[kioskCategory] || [];
       ...
       <>
         ...
         <div className="screensaver-fullscreen" style={{ background: '#000' }}>
            ...
            <div style={{ position: 'absolute', top: '25%', textAlign: 'center', padding: '0 40px', zIndex: 10 }} key={`text-${currentSlide}`} className="slide-enter">
               {activeSlides[currentSlide].title && <h1 style={{ fontSize: 64, fontWeight: 900, color: '#fff', textShadow: '0 4px 20px rgba(0,0,0,0.8)', margin: 0, textTransform: 'uppercase', letterSpacing: '2px' }}>{activeSlides[currentSlide].title}</h1>}
               <div style={{ width: 100, height: 6, background: theme.accent, margin: '24px auto', borderRadius: 4 }}></div>
               {activeSlides[currentSlide].desc && <p style={{ fontSize: 28, color: '#f1f5f9', maxWidth: 800, margin: '0 auto', lineHeight: 1.5, textShadow: '0 4px 20px rgba(0,0,0,0.8)', fontWeight: 600 }}>{activeSlides[currentSlide].desc}</p>}
            </div>
            <div style={{ position: 'absolute', bottom: 120, zIndex: 20 }}><button onClick={goHome} className="kiosk-pulse-btn" style={{ padding: '24px 64px', borderRadius: 100, fontSize: 32, fontWeight: 900, cursor: 'pointer' }}>Interact with inCITe</button></div>
         </div>
       </>
     );
  }
    ...
    <>
      ...
      <div className="screensaver-fullscreen">
        ...
        <div style={{ position: 'absolute', top: 24, left: 32, zIndex: 100, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={goHome}>
            <Bot size={40} color={dark ? "#fff" : theme.cardBorder} /><span style={{ fontSize: 32, fontWeight: 900, letterSpacing: '-1px' }}><span style={{ color: theme.text }}>in</span><span style={{ color: '#1257AC' }}>CIT</span><span style={{ color: theme.text }}>e</span></span>
        </div>
        ...
        {screenState === "home" && !kioskCategory && (
          <div className="kiosk-main-scroll no-scrollbar">
            ...
            <div style={{ width: '100%', maxWidth: 680, marginTop: 40, marginBottom: 32, padding: '0 16px' }}>
               <div className="marquee-container" style={{ background: dark ? 'rgba(166, 1, 18, 0.15)' : 'rgba(166, 1, 18, 0.05)', borderColor: dark ? '#A60112' : 'rgba(166, 1, 18, 0.2)', marginBottom: 24 }}><div className="marquee-text" style={{ color: dark ? '#fff' : '#A60112' }}>#ALABBULSU &nbsp; • &nbsp; COMPLIANCE &nbsp; • &nbsp; INTEGRITY &nbsp; • &nbsp; TRANSPARENCY &nbsp; • &nbsp; #ALABBULSU &nbsp; • &nbsp; COMPLIANCE &nbsp; • &nbsp; INTEGRITY &nbsp; • &nbsp; TRANSPARENCY &nbsp; • &nbsp;</div></div>
               ...
               <div className="marquee-container" style={{ background: dark ? 'rgba(253, 181, 28, 0.15)' : 'rgba(245, 170, 42, 0.1)', borderColor: dark ? '#FDB51C' : 'rgba(245, 170, 42, 0.3)', marginTop: 0 }}><div className="marquee-text" style={{ animationDirection: 'reverse', color: dark ? '#FDB51C' : '#F5AA2A' }}>#AMPLIFIEDCIT &nbsp; • &nbsp; COMPLIANCE &nbsp; • &nbsp; INTEGRITY &nbsp; • &nbsp; TRANSPARENCY &nbsp; • &nbsp; #AMPLIFIEDCIT &nbsp; • &nbsp; COMPLIANCE &nbsp; • &nbsp; INTEGRITY &nbsp; • &nbsp; TRANSPARENCY &nbsp; • &nbsp;</div></div>
            </div>
            ...
            <div style={{ width: '100%', padding: '0 24px', position: 'absolute', bottom: 32, maxWidth: 680 }}>
               <div className="glassy-cluster-card kiosk-pulse-btn" onClick={() => { setScreenState("chat"); setKioskCategory(null); setKioskResult(null); }} style={{ margin: 0, padding: 32, minHeight: 180, display: 'flex', justifyContent: 'center', border: 'none' }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}><span style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.2)', padding: '10px 20px', borderRadius: 24, fontSize: 15, fontWeight: 700 }}><MessageSquare size={18}/> Ask anything</span><span style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.2)', padding: '10px 20px', borderRadius: 24, fontSize: 15, fontWeight: 700 }}><Search size={18}/> Search records</span></div>
