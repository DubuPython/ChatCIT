...
export default function App() {
  ...
  // --- KIOSK SIMULATOR (Ctrl + K) & VIRTUAL KEYBOARD ---
  useEffect(() => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const mobile = window.innerWidth <= 1024 || simKiosk;
      setIsMobile(mobile);
      if (mobile && sidebarOpen) setSidebarOpen(false); 
      else if (!mobile && !sidebarOpen && !simKiosk) setSidebarOpen(true); 
    };
  ...
  useEffect(() => {
    // Forces Kiosk Mode into mobile hamburger view immediately
    if (simKiosk) {
      setIsMobile(true);
  ...
  return (
    <>
      {/* Kills the stray checkbox safely */}
      <style>{`.theme-toggle-wrapper input[type="checkbox"] { display: none !important; }`}</style>
          ...
          <main style={{ 
            ...
            <header style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", height: TOP_H, padding: "0 16px", flexShrink: 0, borderBottom: isMobile ? `1px solid ${dark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'}` : "none", background: bg, zIndex: 50 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                {/* ADMIN DESKTOP NAV FIX - The Theme Toggle returns when sidebars are closed! */}
                {(!isMobile && !gearMode) && (
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <button onClick={() => setShowBugModal(true)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: 8, background: dark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)", border: "none", color: "#ef4444", cursor: "pointer" }}><Bug size={18} /></button>
                    <button onClick={() => setShowCalendar(true)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: 8, background: dark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)", border: "none", color: "#10b981", cursor: "pointer" }}><Calendar size={18} /></button>
